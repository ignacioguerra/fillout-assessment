...
function Button({ children, variant = 'default', PrependIcon, onClick }: ButtonProps) {
  ...
  return (
    <button className={`group bg-white rounded-lg cursor-pointer outline-0
      ...
      variant-${variant}
      group-not-empty/button-group:not-last:border-r-0 group-not-empty/button-group:not-last:rounded-r-none
      group-not-empty/button-group:not-first:border-l-0 group-not-empty/button-group:not-first:rounded-l-none
    `}
    onClick={onClick}
    >
      <span className="flex items-center rounded-lg gap-1.5 pl-2.5 pr-3 py-1.25 transition-color duration-100 h-full w-full
        ...
        group-not-empty/button-group:group-not-last:rounded-r-none
        group-not-empty/button-group:group-not-first:rounded-l-none
      ">
        {PrependIcon && <span className="text-xl text-primary transition-color duration-100
          group-[.variant-tonal]:group-not-active:group-not-focus-visible:text-gray-400
        ">
          <PrependIcon />
        </span>}
        <span className="block text-sm text-black transition-color duration-100 truncate
